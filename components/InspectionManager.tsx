...
import { Save, ArrowLeft, Plus, Trash2, AlertCircle, Search, Settings, ClipboardList, Printer, CheckSquare, ShieldCheck, Edit2 } from 'lucide-react';
...
type InspectionType = 'receiving' | 'process' | 'final';
...
const InspectionManager: React.FC<InspectionManagerProps> = ({
  ...
  // Permission check for Defect Verification (QC)
  const canVerifyDefect = userRole === UserRole.PRODUCTION_LEADER || 
                          userRole === UserRole.ADMIN || 
                          userRole === UserRole.MANAGER || 
                          userRole === UserRole.INSPECTOR;

  // Inspector can view Process Inspection but cannot edit results (Checklist results)
  ...
  const handleChecklistStatusChange = (index: number, status: 'OK' | 'NG' | 'N/A') => {
    ...
    setChecklistItems(prevItems => {
        ...
        // Toggle Logic: If same status is clicked, revert to 'pending'
        ...
        newItems[index] = { 
            ...currentItem, 
            status: nextStatus,
            // Clear inspector/date if pending
  ...
  const handleAddItem = () => {
      ...
      setTargetMaster([...targetMaster, { ...newItemData }]);
      ...
      return (
          <div className="space-y-4">
              <div className="flex gap-4 border-b border-gray-200 pb-4">
                  <button 
                      onClick={() => setSettingsTab('process')}
                      className={`px-4 py-2 rounded-lg font-bold ${settingsTab === 'process' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}
                  >
                      공정 검사 항목
                  </button>
                  <button 
                      onClick={() => setSettingsTab('final')}
                      className={`px-4 py-2 rounded-lg font-bold ${settingsTab === 'final' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}
                  >
                      최종 검사 항목
                  </button>
              ...
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                  <table className="w-full text-sm">
                      <thead className="bg-gray-50 border-b border-gray-200">
                          <tr>
                              <th className="p-3 text-center w-32">{settingsTab === 'process' ? '공정분류' : '1Level'}</th>
                              <th className="p-3 text-center w-32">{settingsTab === 'process' ? '구분' : '2Level'}</th>
                              <th className="p-3 text-left">검사 내용</th>
                              <th className="p-3 text-left w-1/3">검사 기준</th>
                      ...
                      <tbody className="divide-y divide-gray-100">
                          {targetMaster.map((item, index) => (
                              <tr key={index} className="hover:bg-gray-50">
                                  <td className="p-3 text-center font-medium">{item.category}</td>
                                  <td className="p-3 text-center text-gray-500">{item.subCategory}</td>
                                  <td className="p-3 whitespace-pre-wrap">{item.item}</td>
                                  <td className="p-3 text-gray-500 whitespace-pre-wrap text-xs">{item.criteria}</td>
                                  <td className="p-3 text-center">
                                      <div className="flex justify-center gap-2">
                                          <button onClick={() => openEditModal(item, index)} className="p-1 text-gray-400 hover:text-blue-600"><Edit2 size={16}/></button>
                                          <button onClick={() => handleDeleteItem(index)} className="p-1 text-gray-400 hover:text-red-600"><Trash2 size={16}/></button>
                                      </div>
                                  </td>
                              </tr>
                          ))}
              ...
              {isEditModalOpen && (
                  <div className="modal-backdrop">
                      <div className="modal-panel">
                          <h3 className="text-lg font-bold mb-4">{editingItem ? '항목 수정' : '새 항목 추가'}</h3>
                          <div className="space-y-3">
                              <div>
                                  <label className="block text-xs font-bold text-gray-500 mb-1">카테고리 (대분류)</label>
                                  <input type="text" value={newItemData.category} onChange={e => setNewItemData({...newItemData, category: e.target.value})} className="form-control" placeholder="예: 외관/구조" />
                              </div>
                              <div>
                                  <label className="block text-xs font-bold text-gray-500 mb-1">서브 카테고리 (중분류)</label>
                                  <input type="text" value={newItemData.subCategory} onChange={e => setNewItemData({...newItemData, subCategory: e.target.value})} className="form-control" placeholder="예: 치수" />
                              </div>
                              <div>
                                  <label className="block text-xs font-bold text-gray-500 mb-1">검사 내용</label>
                                  <textarea value={newItemData.item} onChange={e => setNewItemData({...newItemData, item: e.target.value})} className="form-control h-20" placeholder="검사할 내용을 입력하세요" />
                              </div>
                              <div>
                                  <label className="block text-xs font-bold text-gray-500 mb-1">검사 기준</label>
                                  <textarea value={newItemData.criteria} onChange={e => setNewItemData({...newItemData, criteria: e.target.value})} className="form-control h-20" placeholder="판정 기준을 입력하세요" />
                              </div>
                          </div>
                          <div className="flex gap-2 mt-6">
                              <button onClick={() => setIsEditModalOpen(false)} className="btn btn-outline flex-1">취소</button>
                              <button onClick={editingItem ? handleUpdateItem : handleAddItem} className="btn btn-primary flex-1">저장</button>
                          </div>
                      </div>
                  </div>
              )}
  ...
  // --- Render Logic for Matrix View (Split Button) ---
  const getStatusColor = (project: Project, panelId: number, type: 'process' | 'final') => {
      ...
      // Strict Check: All items must be OK or N/A (and Checked if boolean)
      if (type === 'process') {
           const applicable = inspection.checkList.filter(item => item.status !== 'N/A');
           if (applicable.length === 0) return '#f97316'; // No items checked yet

           // Process Inspection requires QC Sign off for completion
           const allQc = applicable.every(item => item.qcInspector && item.qcInspector.length > 0);
           if (allQc && inspection.result === 'pass') return '#16a34a'; // Green
          ...
          if (allOk && inspection.result === 'pass') return '#16a34a'; // Green
          return '#f97316'; // Orange
      }
  };

  const getStatusTextColor = (bgColor: string) => {
      return bgColor === '#ffffff' ? '#374151' : '#ffffff';
  };
      ...
      return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              <div className="page-header">
                  <h2 className="page-title flex items-center gap-2"><Settings/> 검사 항목 관리</h2>
              </div>
  ...
  return view === 'list' ? (
                      ...
                      <div className="flex flex-col md:flex-row">
                          <div className="md:w-80 bg-slate-50 p-6 border-r border-gray-200 flex flex-col justify-center">
                              ...
                              <div className="text-sm font-bold text-slate-500 mb-4">
                                  {project.id} <span className="mx-1">/</span> <span className="text-blue-600">{project.taskNumber}</span>
                              </div>
                              <div className="space-y-1 text-sm text-gray-600">
                                  <div className="flex items-center gap-2">
                                      <span className="w-20 font-bold text-gray-400">기종</span>
                                      <span className="font-medium bg-white px-2 py-0.5 rounded border border-gray-200">{project.modelType || '-'}</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                      <span className="w-20 font-bold text-gray-400">납기</span>
                          ...
                          <div className="flex-1 p-6">
                              <div className="mb-3 flex justify-between items-center">
                                  <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">
                                      {isWorker ? '공정 검사 현황 (Panels)' : '검사 현황 (Panels) (좌:공정|우:최종)'}
                                  </span>
                                  <div className="flex gap-2 text-xs">
                                      <span className="flex items-center gap-1"><span className="w-2 h-2 bg-white border border-gray-300 rounded-full"></span> 미검사</span>
                                      <span className="flex items-center gap-1"><span className="w-2 h-2 bg-orange-500 rounded-full"></span> 진행중</span>
                                      <span className="flex items-center gap-1"><span className="w-2 h-2 bg-green-600 rounded-full"></span> 완료</span>
                                  </div>
                              </div>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }} className="inspection-form-container">
          {/* Print Toast */}
          {showPrintToast && (
            <div className="fixed top-5 right-5 bg-slate-800 text-white px-6 py-4 rounded-lg shadow-2xl z-50 animate-bounce flex items-center gap-3 no-print">
                <Printer size={24} className="text-blue-400" />
                <div>
                    <p className="font-bold">인쇄 미리보기</p>
                    <p className="text-sm text-slate-300">창이 열리지 않으면 <span className="text-yellow-400 font-bold">Ctrl + P</span>를 누르세요.</p>
                </div>
            </div>
          )}
          ...
          <div className="text-center mt-4 mb-6">
              <h2 className="text-2xl font-bold text-gray-900">배전반 {activeInspectionType === 'process' ? '공정' : '인수/중간'} 검사 성적서</h2>
          </div>
          ...
          <div className="card">
              {localPanelContext && (
                  <div style={{ marginBottom: '1.5rem', paddingBottom: '1.5rem', borderBottom: '1px solid #f3f4f6' }}>
                       <div className="flex flex-col gap-2">
                          <div className="flex items-center gap-2">
                              <span className="badge badge-indigo">{localPanelContext.project.id}</span>
                              <span className="badge badge-gray">{localPanelContext.project.taskNumber}</span>
                          </div>
                          ...
                          <div className="flex flex-col gap-2 mt-2">
                              <div className="text-gray-500 text-sm font-bold flex items-center gap-1">
                                  ...
                                  기종: {localPanelContext.project.modelType || '-'}
                              </div>
                              <div>
                                  <span className="font-extrabold text-2xl text-blue-700 bg-blue-50 px-4 py-2 rounded-lg border border-blue-200 inline-block shadow-sm">
                                      Panel #{localPanelContext.panelId}
                                  </span>
                              </div>
                          </div>
                      </div>
              ...
              <div>
                  ...
                  <div className="mt-8 pt-6 border-t border-gray-100 no-print">
                      ...
                      <div className="w-full p-4 rounded-lg text-center font-bold text-white shadow-md transition-colors duration-300 flex items-center justify-center gap-2"
                           style={{
                               backgroundColor: formState.result === 'pass' ? '#16a34a' : '#f97316', 
                               height: '60px',
                               fontSize: '1.2rem'
                           }}
                      >
                       ...
                       <button 
                          ...
                          disabled={isSaving}
                          className="btn btn-primary w-full justify-center no-print"
                          style={{ padding: '0 1rem', marginTop: '1rem', height: '56px', fontSize: '1.1rem' }}
                      >
                          <Save size={20} />
                          {isSaving ? '저장 중...' : '검사 완료 저장'}
                      </button>
                  </div>
              </div>
          </div>
